...
function HeroSection({movie}: Readonly<{movie: MovieType }>) {
  const [movieTrailer, setMovieTrailer] = useState<string | null>(null);

  useEffect(() => {
    async function fetchTrailer() {
      try {
        const trailerUrl = await movieService.getTrailerUrl(movie.id);
        if (trailerUrl) {
  ...
  return (
    <div className="w-full h-[500px] flex justify-center items-center md:px-10">
      ...
      >
        {/* Gradient overlay */}
        <div className="absolute inset-0 bg-linear-to-r from-black/80 to-transparent md:rounded-lg"></div>
        ...
        <div className="absolute w-full md:w-xl left-0 bottom-0 p-4 md:p-10 z-10">
          ...
          <button className=" mt-4 px-6 py-2 bg-yellow-500 text-black font-semibold cursor-pointer rounded-4xl hover:bg-yellow-600 transition">
            ...
            <Link href={movieTrailer??'#'} target="_blank" className="text-black">
              Watch Trailer
            </Link>
          </button>
        </div>
